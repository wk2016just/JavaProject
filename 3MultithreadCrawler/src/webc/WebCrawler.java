...
//		wc.addUrl("http://www.cnblogs.com", 1);//加入未爬取和总共爬取了的网页，并且当前深度为1
...
//					while(!notCrawlurlSet.isEmpty()){ ----------------------------------（1）这里会放生多线程错误
//						String tmp = getAUrl();
//						crawler(tmp);
//					}
					while (true) { 
//						System.out.println("当前进入"+Thread.currentThread().getName());
						String tmp = getAUrl();//判断notCrawlurlSet是否为空，不为空则在notCrawlurlSet中get一个再remove它
						if(tmp!=null){
							crawler(tmp);//不为空则开始爬取
						}else{
							synchronized(signal) {  //------------------（2）用于抢占调度线程
								try {
									count++;
									System.out.println("当前有"+count+"个线程在等待");
									signal.wait(); //可以对同一个对象多次wait使其。
									
									//唤醒之后继续执行while true的爬取内容
...
//	}
	...
	public  void crawler(String sUrl){
		...
		try {
				...
				if(d<crawDepth){ //如果是1级d则继续爬取d+1
					//解析网页内容，从中提取链接:这里基本都是从http://www.cnblogs.com开始逐级爬取2级url
...
//			System.out.println(mt.group());
			Matcher myurl = Pattern.compile("href=\".*?\"").matcher(
					mt.group()); //用匹配出来的a连接再次提取其中的href
			while(myurl.find()){
				String str = myurl.group().replaceAll("href=\"|\"", ""); //将href=或者 ""置为空，只留下url地址
//				System.out.println("网址是:"+ str);
				if(str.contains("http:")){ //取出一些不是url的地址
					if(!allurlSet.contains(str)){ //如果是未爬取过的url则
						addUrl(str, dep);//加入一个新的url
						if(count>0){ //如果有等待的线程，则唤醒，这个与之前begin对应！
